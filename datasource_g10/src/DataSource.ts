...
export class DataSource extends DataSourceApi<SignalKQuery, SignalKDataSourceOptions> {
  ...
  async doQuery(options: DataQueryRequest<SignalKQuery>, dataframe: CircularDataFrame, subscriber: Subscriber<DataQueryResponse>) {
      ...
      .then((result: HistoryResult) => {
        ...
        if (result) {
          result.data.forEach((row: number[]) => {
            ...
            dataframe.appendRow([ts, ...row.slice(1).map((value, i) => seriesConversions[i](value))]);
          });
          subscriber.next({
            data: [dataframe],
            key: options.targets[0].refId
  ...
  return (pathValue: PathValue, update: any) => {
    if (pathValue.path === path) {
      if (sourceMatcher(update)) {
        const row = new Array<Date | number | null>(data.fields.length)
        row[0] = new Date();
