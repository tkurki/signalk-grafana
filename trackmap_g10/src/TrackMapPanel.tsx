...
import { MapContainer, GeoJSON, TileLayer, CircleMarker, TileLayerProps, Tooltip, Pane, useMapEvent } from 'react-leaflet';
...
export class TrackMapPanel extends PureComponent<Props, State> {
  ...
  componentDidMount() {
    ...
    this.cursorSubscription = eventBus.getStream(DataHoverEvent).subscribe({
      next: (evt) => {
        if (evt.payload.point.time !== null) {
  ...
  render() {
    ...
    console.log(onBoxZoomEnd)

    function ZoomendHandler() {
      const map = useMapEvent('zoomend', (e) => {
        console.log(map.getBounds())
        onBoxZoomEnd({boxZoomBounds: map.getBounds()})
      })
      return null
    }
    ...
    return (
      <MapContainer bounds={bounds} style={{ width: '100%', height: '100%' }}>
        ...
        {hasDataValues && (
          <Pane name="point-circles" style={{ zIndex: 899 }}>
        ...
        <Pane name="current-point" style={{ zIndex: 999 }}>
